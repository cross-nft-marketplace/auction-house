...
export function auctionDataToPricing(
  ...
  return {
    ...
    auctionCurrencies: auctionData.auctionCurrencies?.map(transformCurrencyEth),
    currentBid: auctionData.currentBid ? {
      ...auctionData.currentBid, 
      currency:  transformCurrencyEth(auctionData.currentBid.currency)
    } : undefined,
    previousBids: auctionData.previousBids?.map((bid) => {
      return {
        ...bid,
...
export function addAuctionInformation(
  ...
  const getBidPricing = (amount: string, currency: Currency): BidPricingInfo => {
    return {
      pricing: {
        currency,
        amount,
  ...
  const nftPricingInformation: PricingInfoData = {
    reserve: pricing.reserve
      ? {
          ...
          currentBid: (() => {
            ...
            return {
              ...bidRaw,
              ...getBidPricing(amount, pricing.reserve?.currentBid?.currency as Currency),
            };
          })(),
          current: {
            highestBid: getHighestReserveBid(),
            likelyHasEnded: pricing.reserve?.status == ReserveAuctionStatus.Finished //ex: finished by BuyNow
              ||
          ...
          reservePrice: {
            ...
            prettyAmount: setCurrencyDecimal(
              ...
              pricing.reserve.reserveAndBuyNowCurrency.decimals
            ),
            computedValue: getCurrencyComputedValue(
              pricing.reserve.reserveAndBuyNowCurrency.id,
              pricing.reserve.reservePrice
            ),
          },
          buyNowPrice: {//possible dont set here or event on indexer layer
            currency: transformCurrencyEth({
              id: pricing.reserve.reserveAndBuyNowCurrency.id,
              name: pricing.reserve.reserveAndBuyNowCurrency.name,
              symbol: pricing.reserve.reserveAndBuyNowCurrency.symbol,
          ...
          previousBids:
            pricing.reserve?.previousBids
              ?.map(({ amount, currency, ...previousBid }) => ({
                ...previousBid,
                ...getBidPricing(amount, currency as Currency),
              }))
              .sort((bidA, bidB) =>
    ...
    auctionType:
      pricing.reserve?.status === 'Active'//wtf, completed?
        ? AuctionType.RESERVE
        : pricing.perpetual
