...
Initial data is used for server-side hydration. If there is no `initialData`, the required data will be fetched client-side by the `@cross-nft-marketplace/auction-house-nft-hooks` library.
